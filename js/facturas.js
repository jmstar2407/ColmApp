...
export async function cargarFacturas() {
  const q    = query(collection(db, 'negocios', state.negocioId, 'facturas'), orderBy('fecha', 'desc'), limit(100));
  const snap = await getDocs(q);
  state.facturasCache = snap.docs.map(d => ({ id: d.id, ...d.data() }));
  ...
  tbody.innerHTML = facturas.map(f => {
    ...
    return `<tr>
      ...
      <td>
        ...
        ${f.estado === 'pendiente' ? `<button class="btn-sm verde" onclick="marcarPagada('${f.id}')" style="padding:6px 10px;font-size:12px;margin-left:4px;"><i class="fas fa-check"></i> Pagar</button>` : ''}
      </td>
    </tr>`;
  }).join('');
}
  ...
  try {
    ...
    if (f && state.cajaActual) {
      await addDoc(collection(db, 'negocios', state.negocioId, 'movimientos'), {
        ...
        fecha: serverTimestamp(), uid: state.currentUser.uid, empleadoNombre: await getEmpNombre(),
